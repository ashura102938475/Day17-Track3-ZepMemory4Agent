...
4. Chat box continues as that user/thread: append turns, retrieve again, keep
   history, and the assistant reply is generated by the configured LLM
   (NVIDIA NIM by default, Gemini fallback) grounded in the retrieved memory
   context.
...
from src.llm import gemini_available, generate_reply, nvidia_available
...
def retrieve_for_case(
    ...
    user = next(u for u in dataset["users"] if u["user_id"] == case["user_id"])
    session = next(
        (s for s in user.get("sessions", []) if s["thread_id"] == case["thread_id"]),
        None,
    )

    short_term = ShortTermMemory(strategy="sliding", max_recent_messages=6, pressure_tokens=450)
    source_messages = case.get("fixture_messages") or (session or {}).get("messages", [])
    ...
    requested = case.get("retrieve_layers")
    if requested is None:
        requested = [case.get("expected_layer", "")]
    if case.get("expected_layer") == "mixed" and not case.get("retrieve_layers"):
        requested = ["long_term", "semantic"]
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.markdown(("✅" if nvidia_available() else "⚠️") + " NVIDIA key "
                    + ("configured" if nvidia_available() else "missing"))
        st.markdown(("✅" if gemini_available() else "⚠️") + " Gemini key "
                    + ("configured" if gemini_available() else "missing"))
        st.caption(f"Chat model: `{settings.nvidia_model if nvidia_available() else settings.gemini_model}`")
        st.divider()
    ...
    if prompt:
        ...
        try:
            ...
            if nvidia_available() or gemini_available():
                reply = generate_reply(context, st.session_state.chat[:-1], prompt)
            else:
                reply = ("_(No LLM key configured — showing retrieved context instead)_\n\n"
